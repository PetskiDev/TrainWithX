...
export async function getPlansFromCreatorSubController(
  req: Request,
  res: Response
) {
...
export async function getPlanSubSlugContent(req: Request, res: Response) {
  ...
  const purchased = await checkUserPurchasedPlan({
    userId: user.id,
    planId: plan.id,
  });
  if (!purchased) {
...
export async function getPlanContentByIdController(
  req: Request,
  res: Response
) {
  const user = req.user!;
...
export async function getMyPurchasedPlansController(
  req: Request,
  res: Response
) {
