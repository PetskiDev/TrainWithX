...
import type { CreatorApplicationDTO } from '@trainwithx/shared';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const AdminDashboard = () => {
  ...
  const filteredAndSortedUsers = users
    ...
    .sort((a, b) => {
      switch (userSortBy) {
        ...
        case 'role':
          return getUserRole(a).localeCompare(getUserRole(b));
        case 'joinDate':
          return (
  ...
  const handleApproveCreator = async (application: CreatorApplicationDTO) => {
    try {
      const res = await fetch(
        `/api/v1/admin/creator-applications/${application.id}/approve`,
        {
          method: 'PATCH',
          credentials: 'include',
          headers: {
  ...
  const handleRejectCreator = async (application: CreatorApplicationDTO) => {
    try {
      const res = await fetch(
        `/api/v1/admin/creator-applications/${application.id}/reject`,
        {
          method: 'PATCH',
          credentials: 'include',
          headers: {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6 mb-8">
          ...
          <Card className="shadow-lg hover:shadow-xl transition duration-300">
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                +{stats.newCreators} this month
              </p>
              <p className="text-xs text-muted-foreground">
                {((100 * stats.totalCreators) / stats.totalUsers).toPrecision(
                  3
                )}
                % of users
              </p>
          ...
          <Card className="shadow-lg hover:shadow-xl transition duration-300">
            ...
            <CardContent>
              <div className="text-3xl font-extrabold text-black">
                ${stats.newRevenue.toLocaleString()}
              </div>
              <p className="text-xs text-muted-foreground">
                  ...
                  <TableBody>
                    {filteredAndSortedPlans.map((plan) => (
                      <TableRow key={plan.id}>
                        <TableCell className="text-left font-medium w-64">
                          {plan.title}
                        </TableCell>
                        <TableCell className="text-left">
                          {plan.creatorUsername}
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-2">
                            <Button
                              ...
                              onClick={() =>
                                goToCreator({
                                  subdomain: plan.creatorSubdomain,
                                  path: `/${plan.slug}`,
                                  newTab: true,
                                })
                              }
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                goPublic(`/plans/edit/${plan.id}`, true)
                              }
                            >
                              <Edit className="h-4 w-4" />
                            </Button>
                  ...
                  {filteredApplications.length > 0 ? (
                    filteredApplications.map((application) => {
                      const isApproved =
                        ...
                        approvedIds.has(application.id);
                          ...
                          <div className="space-y-4">
                                ...
                                <div>
                                  ...
                                  <p className="font-medium text-sm mb-1">
                                    ...
                                    <Link
                                      ...
                                      to={`https://instagram.com/${application.instagram}`}
                                    >
                            ...
                            <div className="grid md:grid-cols-2 gap-4">
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  {application.subdomain}.trainwithx.com
                                </p>
                              </div>
                              <div>
                            ...
                            <div className="border-t pt-4">
                              <div className="flex gap-3 items-end">
                                ...
                                {isApproved ? (
                                  ...
                                  <Button
                                    onClick={() =>
                                      handleApproveCreator(application)
                                    }
                                    className="min-w-32"
                                  >
                                ...
                                {!isApproved && (
                                  <Button
                                    onClick={() =>
                                      handleRejectCreator(application)
                                    }
                                    variant={
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-muted-foreground">
                        There are currently no creator requests to review.
                      </p>
                    </div>
                  )}
                </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex gap-4 mb-6">
                  ...
                  <Select
                    value={creatorSortBy}
                    onValueChange={setCreatorSortBy}
                  >
                    <SelectTrigger className="w-48">
                      <SelectValue placeholder="Sort by..." />
                    </SelectTrigger>
                    ...
                    {filteredAndSortedCreators.map((creator) => (
                      <TableRow key={creator.id}>
                        ...
                        <TableCell className="text-center">
                          <div>
                            <div className="text-lg font-semibold">
                              ${creator.revenueThisMonth.toFixed(2)}
                            </div>
                            <div className="text-xs text-muted-foreground">
                              This Month
                            </div>
      ...
      <Dialog open={showPromoteDialog} onOpenChange={setShowPromoteDialog}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmitPromotion}>Promote</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
